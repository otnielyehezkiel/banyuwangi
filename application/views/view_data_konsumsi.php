        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <div class="row">
                    ...
                    <div class="col-sm-3">
                        <div class="form-group">
                            <label for="bulan_data">Bulan:</label>
                            <select class="form-control" id="bulan_data" onchange="flag();reloaddata();">
                    ...
                    <div class="col-sm-3">
                        <div class="form-group">
                            <label for="tahun_data">Tahun:</label>
...
//    }
    ...
    function bulandata() {
        var $select = $('#bulan_data');
        $.getJSON("<?php echo site_url('data/getbulan')?>"+"/"+'<?php echo $table?>', function(data){
            ...
            //iterate over the data and append a select option
            $.each(data, function(key, val){
                $select.append('<option value="' + val.bulan_data + '">' + val.bulan_data +'</option>')
            });
        });
    }
...
//    var data;
    ...
    function reloaddata()
    {
        ...
        var $select4 = $('#bulan_data');

        table.ajax.url("<?php echo site_url()?>/data/<?php echo $table?>"+"/"+$select1.val()+"/"+$select2.val()+"/"+$select3.val()+"/"+$select4.val()).load();
    ...
    function line_chart(url)
    {
        ...
        $.getJSON(url, function(result){
            ...
            for(i=0;i<result[0].length-1;i++)
            {
                var arr2=Array();
                color = dynamicColors();
                arr2['borderColor'] = color;
                arr2['fill']=false;
                arr2['label']=colum_name[i];
                arr2['tension'] = 0;
                arr2['data']=tmp[i];
                arr2['backgroundColor']=color;
                data2.push(arr2);
